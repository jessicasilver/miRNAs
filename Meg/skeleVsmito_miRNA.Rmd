...
miRNAdatasets = readRDS("rds/miRNAdatasets_forDE.RDS")
...
keep <- rowSums(counts(dds) >= 100) >=26  
dds <- dds[keep,]
...
write.csv(mito.res, file="data/mito_miRNA_DE.csv", row.names=TRUE)
```
# Visualisations
...
hmap.mito.g <- hmap_colGrouped(25, mito.res, count_mat, 
                               hmap_name, sampling_group, sampling_group_col)
```
...
pdf(file = "figures/miRNAs_hmap.pdf",   
